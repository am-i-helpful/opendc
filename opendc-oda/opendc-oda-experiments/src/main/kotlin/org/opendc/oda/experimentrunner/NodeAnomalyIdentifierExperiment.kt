...
fun main() {
    NodeAnomalyIdentifierExperiment().triggerExperiment()
}

//@DisplayName("ODANodeAnomalyIdentificationExperimentRunnerClass")
class NodeAnomalyIdentifierExperiment {
    fun triggerExperiment(){
        val traceFile: String = "/shell_parquet"
        val policy = "FIFO"
        val percentages: List<Int> = listOf<Int>(100, 150)
        val anomalousExperimentalConditions: List<Boolean> = listOf<Boolean>(true, false)
    ...
    private fun conductAnomalyRelatedExperiment(traceFile: String, policy: String, percentage: Int, isAnomalousExperiment: Boolean) = runSimulation {
        val computeService = "compute.opendc.org"
        val workflowService = "workflow.opendc.org"
        // we have fixed the taskOrderPolicy below for anomaly-related experiment for simplicity reasons
        val taskOrderPolicy: TaskOrderPolicy = SubmissionTimeTaskOrderPolicy()
...
//            val isFaultInjected = isAnomalousExperiment
            println("Faulty experimental run is configured = ${isAnomalousExperiment}")
            val monitor = ODAComputeMonitor(traceFile, policy, percentage, isAnomalousExperiment)
            provisioner.runSteps(
                // Configure the ComputeService that is responsible for mapping virtual machines onto physical hosts
                setupComputeService(computeService, scheduler, schedulingQuantum = Duration.ofSeconds(1), isFaultInjected = isAnomalousExperiment),
                registerComputeMonitor(serviceDomain = "compute.opendc.org", monitor, exportInterval = Duration.ofSeconds(metricExportingQuantum)),
                setupHosts(computeService, specs, isFaultInjected = isAnomalousExperiment),
